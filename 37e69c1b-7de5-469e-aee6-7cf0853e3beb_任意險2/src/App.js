...
export default function App() {
  ...
  useEffect(() => {
    const fetchHullRateTables = async () => {
      try {
        const [{ data: versions }, { data: claims }] = await Promise.all([
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setPhone("");
      setClientEmail("");
      setStartDate("1150808");
      setStartDateArbitrary("1150808");
      setMergedBrandSeries("");
      setModelCode("");
      setIssueDate("11508");
      setManufactureDate("202608");
      setEngineDisplacement("");
      setReplacementValue("");
      setLevel("4");
      setDrunkCount("0");
      setLiabilityClaimLevel("0");
      setCarDamageLevel("0");
      setHasHull(false);
      setHullType("07丙式");
      setHullDeductible("0");
    ...
    >
      ...
      <div className="row g-2 mb-4">
        <div className="col-3">
          ...
          <button
            type="button"
            className="btn btn-outline-info w-100 fw-bold shadow-sm mt-2"
            onClick={() => setShowFormulaModal(true)}
            disabled={!hullFormulaDetail}
          >
            🔍 查看公式（車體險）
          </button>
        </div>
      ...
      {/* 📁 彈窗二：經辦全體保險資料庫完整一覽表 */}
      {showFormulaModal && hullFormulaDetail && (
        <div
          ...
          <div className="d-flex align-items-center justify-content-center min-vh-100 p-3">
            ...
            >
              ...
              <div className="small">
                ...
                <div className="d-flex justify-content-between border-bottom py-1">
                  <span>③ 基本費率</span>
                  <span>{(hullFormulaDetail.baseRate * 100).toFixed(2)}%</span>
                </div>
                <div className="d-flex justify-content-between border-bottom py-1">
                  <span>④ 基本保費（②×③）</span>
                  <span>
              ...
              <div className="bg-light rounded-2 p-2 mt-2 text-danger fw-bold text-center">
                最終保費 = NT$ {hullFormulaDetail.finalPremium.toLocaleString()}
              </div>
            </div>
          </div>
        </div>
